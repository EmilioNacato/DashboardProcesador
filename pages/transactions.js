...
export default function Transactions() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const expectedUrl = `http://localhost:8081/api/v1/transacciones/recientes?desde=${encodeURIComponent(`${dateRange.desde.fecha}T${dateRange.desde.hora}:00`)}&hasta=${encodeURIComponent(`${dateRange.hasta.fecha}T${dateRange.hasta.hora}:59`)}`;
